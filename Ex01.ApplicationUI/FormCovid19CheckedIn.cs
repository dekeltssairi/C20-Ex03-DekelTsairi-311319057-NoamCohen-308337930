...
namespace Ex01.ApplicationUI
{
    public partial class FormCovid19CheckedIn : Form
    {
        ...
        {
            loadConfirmedSickLocations();
        }

        private void loadConfirmedSickLocations()
        {
            ResultStatus_label.Text = " ";
        ...
        {
            sortConfirmedSickLocations(new ByABCSorter());
        }

        private void radioButtonTime_CheckedChanged(object sender, EventArgs e)
        {
            sortConfirmedSickLocations(new ByDateAndTimeSorter());

            //r_ConfirmedSickPeople.ConfirmedSickLocations.Sort((x, y) => x.DateOfSickConfirmation.CompareTo(y.DateOfSickConfirmation));
        }

        private void sortConfirmedSickLocations<T>(T i_Strategy)
        {
            CheckinsSorter sorter = new CheckinsSorter(i_Strategy as ISortStrategy);
            sorter.Sort(r_ConfirmedSickPeople.ConfirmedSickLocations);
